...
void prim(G *graph, int source, vector<int> &distance) { //&distance: vector de distâncias mínimas passado como referência de um valor, ou seja, altera o valor original do vector, sem tratar de uma cópia
    for (int i = 0; i < graph->size; ++i) {
    ...
    while (!heap.empty()) {
        ...
        while (w < graph->size) { // itera pelos adjacentes enquanto houver vértices a serem explorados
            if (getMark(graph,w) != VISITED and distance[w] > weight(graph, vertex, w)) {
                distance[w] = weight(graph, vertex, w);
